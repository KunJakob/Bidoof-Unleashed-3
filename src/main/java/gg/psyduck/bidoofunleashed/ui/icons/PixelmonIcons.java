...
public class PixelmonIcons {
	...
	public static ItemStack applySpecDetails(ItemStack picture, BU3PokemonSpec spec) {
		...
		if(spec.item != null) {
			lore.add(Text.of(TextColors.GRAY, "Holding: ", TextColors.YELLOW, Sponge.getRegistry().getType(ItemType.class, spec.item).orElse(ItemTypes.AIR).getTranslation().get()));
		}

		if (spec.extraSpecs != null) {
			if (spec.extraSpecs.stream().anyMatch(s -> s.key.startsWith("ev"))) {
				int[] evs = new int[6];
				for (int i = 0; i < 6; i++) {
					evs[i] = -1;
				}
			...
			if(spec.extraSpecs.stream().anyMatch(s -> s.key.startsWith("iv"))) {
				int[] ivs = new int[6];
				for (int i = 0; i < 6; i++) {
					ivs[i] = -1;
				}
