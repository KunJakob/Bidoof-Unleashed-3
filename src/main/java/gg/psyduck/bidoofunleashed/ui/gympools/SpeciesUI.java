...
class SpeciesUI implements PageDisplayable {
	...
	private Page build() {
		...
		if((challenger == null ? this.focus.getBattleSettings(EnumBattleType.First) : this.focus.getBattleSettings(this.focus.getBattleType(challenger))).getPool().getTeam().size() > 21) {
